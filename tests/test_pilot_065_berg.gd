## test_pilot_065_berg.gd - 柏格（pilot_065，联邦 N）效果测试
##
## 柏格 1 个主动效果按钮（DIRECT）：
...
func _setup_boge(battle, owner_id: StringName):
	...
	if mech == null:
		return {}
	var card = _make_instance(gs, cdb, "pilot_065_柏格", owner_id)
	if card == null:
		return {}
...
func _fire_pilot_065(battle, pilot_card, mech, player_id: StringName, effect_id: StringName) -> _Action:
	var src: Dictionary = {
...
## 测试1：效果定义正确
func test_pilot_065_effect_definitions() -> Variant:
	var effects = _ActionPilotEffects.build_pilot_effects()
	var e1 = effects.get(&"pilot_065_effect_01")
	if e1 == null:
		return "缺 pilot_065_effect_01"
	if e1.mode != _TimingConst.MODE_DIRECT:
		return "effect_01 mode 应 MODE_DIRECT 实=%s" % String(e1.mode)
	if e1.once_per_turn_key != &"pilot_065_effect_01":
		return "once_per_turn_key 应 pilot_065_effect_01"
	...
	if String(cm_p.get("store_result_key", &"")) != "pilot_065_discard_ids":
		return "effect_01 store_result_key 应 pilot_065_discard_ids"
	...
	if String(ca_conds[0].get("params", {}).get("key", &"")) != "pilot_065_discard_ids":
		return "effect_01 分支条件 key 应 pilot_065_discard_ids"
...
func test_pilot_065_discard_part_gold_draw_equipment() -> Variant:
	...
	var ef = await _fire_pilot_065(battle, s.pilot_card, s.mech, &"player", &"pilot_065_effect_01")
	if ef == null:
...
func test_pilot_065_discard_weapon_bonus_actions() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_boge(battle, &"player")
...
func test_pilot_065_no_unequipped_but_slot_gray() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_boge(battle, &"player")
...
func test_pilot_065_cancel_abort_no_consume() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_boge(battle, &"player")
	...
	if _equip_deck_size(battle) != deck_before:
		return "取消不应抽牌"
	# 次数未消耗：可再触发
	var ef2 = await _fire_pilot_065(battle, s.pilot_card, s.mech, &"player", &"pilot_065_effect_01")
	if ef2 == null:
		return "取消中止后应可再触发"
...
func test_pilot_065_once_per_turn_max_1() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_boge(battle, &"player")
	...
	if c1 == &"" or c2 == &"":
		return "装备牌设置失败"
	# 第一次：完整发动（弃1抽1）
	var ef1 = await _fire_pilot_065(battle, s.pilot_card, s.mech, &"player", &"pilot_065_effect_01")
	if ef1 == null:
		return "第1次未挂起"
