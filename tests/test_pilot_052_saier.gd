## test_pilot_052_saier.gd - 萨伊（pilot_052，秩序 R）效果测试
##
...
func _setup_saier(battle, owner_id: StringName):
	...
	if mech == null:
		return {}
	var card = _make_instance(gs, cdb, "pilot_052_萨伊", owner_id)
	if card == null:
		return {}
...
func _fire_pilot_052(battle, pilot_card, mech, player_id: StringName, effect_id: StringName) -> _Action:
	var src: Dictionary = {
...
## 测试1：效果定义正确
func test_pilot_052_effect_definition() -> Variant:
	var effects = _ActionPilotEffects.build_pilot_effects()
	var e1 = effects.get(&"pilot_052_effect_01")
	if e1 == null:
		return "缺 pilot_052_effect_01"
	if e1.mode != _TimingConst.MODE_DIRECT:
		return "effect_01 mode 应 MODE_DIRECT 实=%s" % String(e1.mode)
	if e1.once_per_turn_key != &"pilot_052_effect_01":
		return "once_per_turn_key 应 pilot_052_effect_01"
	...
	if String(cm_p.get("store_result_key", &"")) != "pilot_052_discard_ids":
		return "effect_01 store_result_key 应 pilot_052_discard_ids"
...
func test_pilot_052_full_flow_discard_draw() -> Variant:
	...
	var ef = await _fire_pilot_052(battle, s.pilot_card, s.mech, &"player", &"pilot_052_effect_01")
	if ef == null:
...
func test_pilot_052_cancel_abort_no_consume() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_saier(battle, &"player")
	...
	if _equip_deck_size(battle) != deck_before:
		return "取消装备牌堆不应变化"
	# 次数未消耗：可再触发
	var ef2 = await _fire_pilot_052(battle, s.pilot_card, s.mech, &"player", &"pilot_052_effect_01")
	if ef2 == null:
		return "取消中止后应可再触发"
...
func test_pilot_052_once_per_turn_max_2() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_saier(battle, &"player")
	...
	var equip_hand_before: int = _equip_hand_size(battle, &"player")
	# 第一次：完整发动（弃1抽1）
	var ef1 = await _fire_pilot_052(battle, s.pilot_card, s.mech, &"player", &"pilot_052_effect_01")
	if ef1 == null:
		return "第1次未挂起"
	...
	var deck_before: int = _equip_deck_size(battle)
	var ef3 = await _fire_pilot_052(battle, s.pilot_card, s.mech, &"player", &"pilot_052_effect_01")
	if ef3 != null:
...
func test_pilot_052_no_action_card_gray() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_saier(battle, &"player")
...
func test_pilot_052_owner_actions_across_players() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	...
	var ef = await _fire_pilot_052(battle, s.pilot_card, s.mech, &"third", &"pilot_052_effect_01")
	if ef == null:
