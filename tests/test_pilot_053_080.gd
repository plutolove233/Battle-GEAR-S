## test_pilot_053_080.gd - 李（pilot_053）/ 墨尘（pilot_080）效果测试
##
## 李 pilot_053（秩序 R，2 按钮：主动+被动合一，悬停说明）：
...
func test_pilot_053_definitions() -> Variant:
	...
	var e2 = all.get(&"pilot_053_effect_02")
	if e2 == null:
		return "缺 pilot_053_effect_02"
	...
	if e2.once_per_game_key != &"pilot_053_effect_02":
...
func test_pilot_053_e1_draw_set_once_per_turn() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot(battle, &"player", "pilot_053_李")
	if s == null:
		return "setup 失败（缺 pilot_053_李）"
	var gs = s.gs
	...
	var ef = await _fire_effect(battle, s.card, s.mech, &"player", &"pilot_053_effect_01")
	...
	await _wait_effect_fire_done(battle, &"pilot_053_effect_01", 100)
	...
	var ef2 = await _fire_effect(battle, s.card, s.mech, &"player", &"pilot_053_effect_01")
...
func test_pilot_053_e1_deck_empty_blocked() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot(battle, &"player", "pilot_053_李")
...
func test_pilot_053_e2_discard_branch() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot(battle, &"player", "pilot_053_李")
...
func test_pilot_053_e2_transfer_branch() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot(battle, &"player", "pilot_053_李")
...
func test_pilot_053_e2_cancel_not_consumed() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot(battle, &"player", "pilot_053_李")
