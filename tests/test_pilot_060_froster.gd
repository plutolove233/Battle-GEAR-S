## test_pilot_060_froster.gd - 冰魄（pilot_060）效果测试
##
...
func _setup_bingpo(battle, owner_id: StringName):
	...
	var card = _make_instance(gs, cdb, "pilot_060_冰魄", owner_id)
	if card == null:
		return null
...
func test_pilot_060_effect_01_definition() -> Variant:
	...
	if String(acts[1].get("params", {}).get("flag", &"")) != "pilot_060_range_reduced":
		return "SET_ACTION_RECORD_FLAG flag 应 pilot_060_range_reduced"
	return true


## 测试2：effect_02 定义正确（LISTEN ATTACK_AFTER，hide_button + merge_desc_into_index=1，flag+miss 条件，抽2）
func test_pilot_060_effect_02_definition() -> Variant:
	var e = _ActionPilotEffects.build_pilot_effects().get(&"pilot_060_effect_02")
	if e == null:
		return "缺 pilot_060_effect_02"
	if e.mode != _TimingConst.MODE_LISTEN:
	...
	for c in e.conditions:
		if String(c.get("op", &"")) == "ATTACK_RECORD_FLAG_IS_SET":
			if String(c.get("params", {}).get("flag", &"")) != "pilot_060_range_reduced":
				return "ATTACK_RECORD_FLAG_IS_SET flag 应 pilot_060_range_reduced"
...
func test_pilot_060_e01_counter_reduce_range_and_flag() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_bingpo(battle, &"player")
	if s == null:
		return "setup 失败（缺 pilot_060_冰魄）"
	var gs = s.gs
	var mech = s.mech
	...
	if not bool(flags.get("pilot_060_range_reduced", {}).get("value", false)):
		return "e01 应写 flag pilot_060_range_reduced 实=%s" % str(flags)
	return true


## 测试4：e01 范围钳制——基础范围1时 -2 后仍不低于1（extra_range 钳制为0）
func test_pilot_060_e01_clamp_min_range() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_bingpo(battle, &"player")
...
func test_pilot_060_e01_not_counter_no_trigger() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_bingpo(battle, &"player")
...
func test_pilot_060_e02_miss_draw() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_bingpo(battle, &"player")
	...
	mock_attack.record["_effect_flags"] = {"pilot_060_range_reduced": {"value": true, "data": {}}}
...
func test_pilot_060_e02_hit_no_draw() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_bingpo(battle, &"player")
...
func test_pilot_060_e02_no_flag_no_draw() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_bingpo(battle, &"player")
...
func test_pilot_060_full_chain_reduce_then_miss_draw() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_bingpo(battle, &"player")
