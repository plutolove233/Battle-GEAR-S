...
## 闪击：选牌续跑后应再发动一次攻击 B（EXECUTE_ATTACK use_previous_weapon/target）
func test_flash_real_resume_triggers_second_attack():
	...
	# 玩家选 fodder1 弃牌续跑（触发 attack B：完整 attack 动作，预填武器/目标，跳过选择）
	...
	# attack B 作为完整 attack 动作执行：命中并造成伤害，敌方 HP 应再次下降
	# （attack B 同步跑完 ATTACK_BEFORE→PRE→AT→AFTER→SETTLE，apply_damage 同步扣 HP）
