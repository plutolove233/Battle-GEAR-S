## test_pilot_021_taliyah.gd - 塔莉娅（pilot_021，帝国 SR）效果测试
##
...
func _setup_taliyah(battle, owner_id: StringName = &"player") -> Dictionary:
	...
	if mech == null:
		return {}
	var pilot_card = _set_pilot_on_mech(battle, owner_id, mech, "pilot_021_塔莉娅")
	if pilot_card == null:
		return {}
	return {
		"gs": gs,
...
func _fire_pilot_021(battle, pilot_card, mech, player_id: StringName) -> _Action:
	var src: Dictionary = {
	...
	# 子动作完成由 _continue_pilot_021_draw 续跑才挂起选机甲窗（waiting_timing）。循环等待。
	for i in range(40):
...
func test_p021_definitions() -> Variant:
	...
	# effect_01 主动抽3赐予
	var e1 = effs.get(&"pilot_021_effect_01")
	if e1 == null:
		return "缺 pilot_021_effect_01"
	if e1.mode != _TimingConst.MODE_DIRECT:
		return "effect_01 mode 应 DIRECT 实=%s" % String(e1.mode)
	if e1.once_per_turn_key != &"pilot_021_effect_01":
		return "effect_01 once_per_turn_key 应 pilot_021_effect_01"
	...
	var e2 = effs.get(&"pilot_021_effect_02")
	if e2 == null:
...
func test_p021_effect1_full_loop_give_all() -> Variant:
	...
	var ef = await _fire_pilot_021(battle, pilot_card, mech, &"player")
	if ef == null:
	...
	for cid in hand:
		if not _ActionPilotEffects.pilot_021_card_has_any_jin(gs.get_card(cid)):
			return "抽的牌应打禁标签 card=%s" % String(cid)

	# 选机甲 -> 选牌（挂起）
	te.resume_pending_effect(ef.action_id, {"target_id": enemy_mech.mech_id})
	await _pump_frames(5)
	if te._pending_effect.get(ef.action_id, {}).get("phase", &"") != &"pilot_021_choose_cards":
		return "选中机甲后应挂起选牌阶段"
	...
	for cid in [hand[0], hand[1]]:
		if not _ActionPilotEffects.pilot_021_card_has_any_ce(gs.get_card(cid)):
			return "交出的牌应打策标签 card=%s" % String(cid)
	if te._pending_effect.get(ef.action_id, {}).get("phase", &"") != &"pilot_021_choose_mech":
		return "给2张后应续选机甲"
	...
	if not _ActionPilotEffects.pilot_021_card_has_any_ce(gs.get_card(hand[2])):
		return "第3张交出的牌应打策标签"
	...
	var ef2 = await _fire_pilot_021(battle, pilot_card, mech, &"player")
	if ef2 != null:
...
func test_p021_effect1_cancel_card_select_rechoose() -> Variant:
	...
	if te._pending_effect.get(ef.action_id, {}).get("phase", &"") != &"pilot_021_choose_mech":
		return "给1张后应续选机甲"
	if not _ActionPilotEffects.pilot_021_card_has_any_ce(gs.get_card(hand[0])):
		return "给出的牌应打策标签"
	te.resume_pending_effect(ef.action_id, {"cancelled": true})
	await _pump_frames(6)
	return true
...
func test_p021_jin_block_and_endturn_clear() -> Variant:
	...
	if not gs.players.get(&"player").action_hand.has(hand[0]):
		return "禁牌使用被拒后应仍留手牌"
	if not _ActionPilotEffects.pilot_021_has_jin(gs.get_card(hand[0]), &"player"):
	...
	await _pump_frames(6)
	for cid in gs.players.get(&"player").action_hand:
		if _ActionPilotEffects.pilot_021_card_has_any_jin(gs.get_card(cid)):
			return "回合结束后禁标签应清除"
	return true
...
func test_p021_ce_draw_on_use_not_direct_discard() -> Variant:
	...
	_ActionPilotEffects.pilot_021_on_card_left_taliyah_hand(c1_card, &"player")
	if not _ActionPilotEffects.pilot_021_card_has_any_ce(c1_card):
		return "c1 应带策标签"
	_force_discard(battle, &"enemy", [c1])
	await _pump_frames(5)
	if not gs.players.get(&"player").action_hand.is_empty():
		return "直接弃置不应触发塔莉娅抽牌 实=%d" % gs.players.get(&"player").action_hand.size()
	if _ActionPilotEffects.pilot_021_card_has_any_ce(c1_card):
	...
	_ActionPilotEffects.pilot_021_on_card_left_taliyah_hand(c2_card, &"player")
	...
	if _ActionPilotEffects.pilot_021_card_has_any_ce(c2_card):
...
func test_p021_transfer_and_steal_apply_ce() -> Variant:
	...
	if not gs.players.get(&"enemy").action_hand.has(c1):
		return "转移后敌方手牌应含 c1"
	if not _ActionPilotEffects.pilot_021_card_has_any_ce(gs.get_card(c1)):
		return "转移交牌应打策标签"
	...
	if not gs.players.get(&"enemy").action_hand.has(c2):
		return "偷牌后敌方手牌应含 c2"
	if not _ActionPilotEffects.pilot_021_card_has_any_ce(gs.get_card(c2)):
		return "偷牌也应打策标签"
	return true
