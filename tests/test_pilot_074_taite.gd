## test_pilot_074_taite.gd - 泰特（pilot_074，帝国 N）效果测试
##
...
##     GRANT_MELEE_MIGHT 向目标机甲注册 DIRECT(虚拟时点 pilot_074_effect_01)+3隐藏LISTEN（binding granted=true，
...
func _setup_taite(battle, owner_id: StringName):
	...
	if mech == null:
		return {}
	var card = _make_instance(gs, cdb, "pilot_074_泰特", owner_id)
	if card == null:
		return {}
...
func _fire_pilot_074_effect1(battle, pilot_card, mech, player_id: StringName) -> _Action:
	var src: Dictionary = {
...
func _fire_pilot_074_effect2(battle, pilot_card, mech, player_id: StringName) -> _Action:
	var src: Dictionary = {
		"card_instance_id": pilot_card.instance_id,
		"mech_id": mech.mech_id,
		"player_id": player_id,
		"effect_id": &"pilot_074_effect_02",
	}
	battle.context.game_state.active_player_id = player_id
	battle.context.game_state.phase = &"MAIN"
	battle.context.action_service.execute(&"effect_fire", {
...
func test_pilot_074_effect_definitions() -> Variant:
	var effects: Dictionary = _ActionPilotEffects.build_pilot_effects()

	# ── effect_01（按钮1 DIRECT）──
	var e1 = effects.get(&"pilot_074_effect_01")
	if e1 == null:
		return "缺 pilot_074_effect_01"
	if e1.mode != _TimingConst.MODE_DIRECT:
		return "e1 mode 应 MODE_DIRECT 实=%s" % String(e1.mode)
	if String(e1.once_per_turn_key) != "pilot_074_effect_01":
		return "e1 once_per_turn_key 应 pilot_074_effect_01"
	...
	if String(e1cm.get("store_result_key", &"")) != "pilot_074_discard_ids":
		return "e1 store_result_key 应 pilot_074_discard_ids"
	...
	var e1a = effects.get(&"pilot_074_effect_01_apply")
	var e1c = effects.get(&"pilot_074_effect_01_consume")
	var e1t = effects.get(&"pilot_074_effect_01_turnend")
	...
	var e2 = effects.get(&"pilot_074_effect_02")
	if e2 == null:
		return "缺 pilot_074_effect_02"
	if e2.mode != _TimingConst.MODE_DIRECT:
		return "e2 mode 应 MODE_DIRECT"
	if String(e2.once_per_turn_key) != "pilot_074_effect_02":
		return "e2 once_per_turn_key 应 pilot_074_effect_02"
	...
	var e2x = effects.get(&"pilot_074_effect_02_expire")
	if e2x == null:
		return "缺 pilot_074_effect_02_expire"
...
func test_pilot_074_discard_accumulate() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_taite(battle, &"player")
	if s.is_empty():
		return "setup 失败（缺 pilot_074_泰特）"
	...
	var ef = await _fire_pilot_074_effect1(battle, s.pilot_card, s.mech, &"player")
	if ef == null:
	...
	# 每回合3次额度消耗1次（3次内仍可用）
	if not battle.context.timing_engine.is_once_per_turn_key_available(&"pilot_074_effect_01", s.pilot_card.instance_id, 3):
...
func test_pilot_074_cancel_no_cost() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_taite(battle, &"player")
	...
	var ef2 = await _fire_pilot_074_effect1(battle, s.pilot_card, s.mech, &"player")
	if ef2 == null:
		return "取消中止后应可再触发"
	return true
...
## 测试4：每回合3次用满 -> 第4次跳过
func test_pilot_074_once_per_turn_3() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_taite(battle, &"player")
	...
	if battle.context.timing_engine.is_once_per_turn_key_available(&"pilot_074_effect_01", s.pilot_card.instance_id, 3):
		return "3次用满后额度应不可用"
	# 第4次触发：once_per_turn 用满 -> 跳过，不挂起、不再弃牌
	var hand_before: int = player.action_hand.size()
	var ef4 = await _fire_pilot_074_effect1(battle, s.pilot_card, s.mech, &"player")
	if ef4 != null:
...
func test_pilot_074_melee_attack_applies() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_taite(battle, &"player")
...
func test_pilot_074_ranged_no_trigger() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_taite(battle, &"player")
...
func test_pilot_074_settle_consumes() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_taite(battle, &"player")
...
func test_pilot_074_turn_after_end_clear() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_taite(battle, &"player")
...
func test_pilot_074_grant_other_mech() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_taite(battle, &"player")
	...
	# 授予敌方机甲
	var ef2 = await _fire_pilot_074_effect2(battle, s.pilot_card, s.mech, &"player")
	if ef2 == null:
	...
	# EX DIRECT（虚拟时点 pilot_074_effect_01）+ 隐藏 LISTEN 注册到敌方机甲（granted=true）
	var ex_mechs: Array = _perm_listener_mechs(ctx, &"pilot_074_effect_01", &"pilot_074_effect_01")
	if not ex_mechs.has(String(enemy_mech.mech_id)):
		return "敌方机甲应注册 EX DIRECT（虚拟时点）"
	var ab_mechs: Array = _perm_listener_mechs(ctx, _TimingConst.ATTACK_BEFORE, &"pilot_074_effect_01_apply")
	if not ab_mechs.has(String(enemy_mech.mech_id)):
		return "敌方机甲应注册 ATTACK_BEFORE apply 监听器"
	var st_mechs: Array = _perm_listener_mechs(ctx, _TimingConst.ATTACK_SETTLE, &"pilot_074_effect_01_consume")
	...
	if tl.has(&"pilot_074_effect_01"):
		for entry: Dictionary in tl[&"pilot_074_effect_01"]:
			var bc: Dictionary = entry.get("binding_context", {})
	...
	# effect_02 每回合1次已消耗
	if battle.context.timing_engine.is_once_per_turn_key_available(&"pilot_074_effect_02", s.pilot_card.instance_id, 1):
		return "授予后每回合1次应已消耗"
...
func test_pilot_074_expire_at_turn_start() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_taite(battle, &"player")
	...
	# 敌方 EX/监听器注销
	if _perm_listener_mechs(ctx, &"pilot_074_effect_01", &"pilot_074_effect_01").has(String(enemy_mech.mech_id)):
		return "到期后敌方 EX DIRECT 应注销"
	if _perm_listener_mechs(ctx, _TimingConst.ATTACK_BEFORE, &"pilot_074_effect_01_apply").has(String(enemy_mech.mech_id)):
		return "到期后敌方 apply 监听器应注销"
	if _perm_listener_mechs(ctx, _TimingConst.ATTACK_SETTLE, &"pilot_074_effect_01_consume").has(String(enemy_mech.mech_id)):
	...
	if not _perm_listener_mechs(ctx, &"pilot_074_effect_01", &"pilot_074_effect_01").has(String(s.mech.mech_id)):
		return "泰特自身 EX DIRECT 应保留"
	if not _perm_listener_mechs(ctx, _TimingConst.ATTACK_BEFORE, &"pilot_074_effect_01_apply").has(String(s.mech.mech_id)):
		return "泰特自身 apply 监听器应保留"
	if not _perm_listener_mechs(ctx, _TimingConst.TURN_AFTER_END, &"pilot_074_effect_01_turnend").has(String(s.mech.mech_id)):
