## test_pilot_022.gd - 提比里安（pilot_022）效果测试
##
## 提比里安 2 按钮：
...
##     写 counters["pilot_022_effect_02_used"] 本局仅一次。
extends RefCounted
...
func _setup_pilot_022(battle, owner_id: StringName):
	var gs = battle.context.game_state
	var cdb = battle.context.card_database
	var mech = gs.get_mech_for_player(owner_id)
	var card = _make_instance(gs, cdb, "pilot_022_提比里安", owner_id)
	if card == null:
		return null
...
func _fire_pilot_022_effect1(battle, pilot_card, mech, player_id: StringName) -> _Action:
	var src: Dictionary = {
...
func test_pilot_022_effect_01_definition() -> Variant:
	var e = _ActionPilotEffects.build_pilot_effects().get(&"pilot_022_effect_01")
	if e == null:
		return "缺 pilot_022_effect_01"
	if e.mode != _TimingConst.MODE_DIRECT:
		return "effect_01 mode 应 MODE_DIRECT 实=%s" % String(e.mode)
	if e.once_per_turn_key != &"pilot_022_effect_01":
		return "once_per_turn_key 应 pilot_022_effect_01"
	...
	if cm.get("store_result_key", &"") != &"pilot_022_weapon":
		return "store_result_key 应 pilot_022_weapon"
	if String(acts[1].get("type", &"")) != "EXECUTE_DISCARD":
		return "actions[1] 应 EXECUTE_DISCARD"
	if acts[1].get("params", {}).get("card_ids", &"") != "$runtime.pilot_022_weapon":
		return "EXECUTE_DISCARD card_ids 应 $runtime.pilot_022_weapon"
...
func test_pilot_022_effect_02_definition() -> Variant:
	var e = _ActionPilotEffects.build_pilot_effects().get(&"pilot_022_effect_02")
	if e == null:
...
func test_pilot_022_e01_discard_weapon_apply_bonus() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot_022(battle, &"player")
	if s == null:
		return "setup 失败（缺 pilot_022_提比里安）"
	var gs = s.gs
	var mech = s.mech
	...
	var ef = await _fire_pilot_022_effect1(battle, s.card, mech, &"player")
	if ef == null:
...
func test_pilot_022_e01_cancel_no_consume() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot_022(battle, &"player")
	...
	var ef2 = await _fire_pilot_022_effect1(battle, s.card, mech, &"player")
	if ef2 == null:
...
func test_pilot_022_e01_no_weapon_button_disabled() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot_022(battle, &"player")
...
func test_pilot_022_e01_bonus_applies_then_clears() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot_022(battle, &"player")
	...
	s.card.counters["pilot_022_effect_02_used"] = true
...
func test_pilot_022_e01_bonus_cleared_on_turn_end() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot_022(battle, &"player")
...
func test_pilot_022_e02_fire_confirm() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot_022(battle, &"player")
	...
	# 本局标记
	if not bool(s.card.counters.get("pilot_022_effect_02_used", false)):
		return "应写 counters[pilot_022_effect_02_used]=true"
	return true


## 测试9：e02 取消发动 → 无威力/范围/锁定/标记
func test_pilot_022_e02_cancel_no_effect() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot_022(battle, &"player")
	...
	if _count_locked(gs, enemy_mech.mech_id) != 0:
		return "取消后不应施加锁定"
	if bool(s.card.counters.get("pilot_022_effect_02_used", false)):
		return "取消后不应写本局标记"
	return true


## 测试10：e02 本局1次 -- 第2次攻击不再触发（PILOT_022_NOT_USED_THIS_GAME）
func test_pilot_022_e02_second_attack_no_trigger() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot_022(battle, &"player")
...
func test_pilot_022_e02_multi_target_lock_all() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot_022(battle, &"player")
