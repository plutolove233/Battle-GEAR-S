## test_pilot_074_viola.gd - 维奥拉（pilot_074）效果测试
##
...
func _setup_viola(battle) -> Dictionary:
	...
	if mech == null:
		return {}
	var card = _make_instance(gs, cdb, "pilot_074_维奥拉", &"player")
	if card == null:
		return {}
...
func test_077_effect_definition() -> Variant:
	...
	if setup.is_empty():
		return "找不到 pilot_074_维奥拉"
	var all_effects: Dictionary = _ActionPilotEffects.build_pilot_effects()
	var e1 = all_effects.get(&"pilot_074_effect_01")
	if e1 == null:
	...
	if String(a0.get("params", {}).get("once_per_turn_key", &"")) != "pilot_074_effect_01":
