## test_pilot_065_yinxue.gd - 银雪（pilot_065，联邦 N，cost 4, attack_limit 1, action_card_limit 3）
##
## 银雪效果「窥牌拦截」（运行时机师效果走 ActionPilotEffects 新体系）：
##   effect_01（DIRECT 显示按钮，开关）：随时按弹"启用/禁用窥牌拦截"二选一（可取消）。
##     flag 存 card.counters["pilot_065_intercept"]（默认启用=true）。CHOOSE_ONE 两选项各带
...
func _setup_yinxue(battle, owner_id: StringName):
	...
	if mech == null:
		return {}
	var card = _make_instance(gs, cdb, "pilot_065_银雪", owner_id)
	if card == null:
		return {}
...
func _fire_pilot_065_toggle(battle, pilot_card, mech, player_id: StringName) -> _Action:
	var src: Dictionary = {
...
## 读取机师牌 counters["pilot_065_intercept"]（不存在按默认 true）
func _intercept_flag(pilot_card) -> bool:
	if pilot_card == null:
		return true
	if not "counters" in pilot_card:
		return true
	return bool(pilot_card.counters.get("pilot_065_intercept", true))
...
func test_p065_definitions() -> Variant:
	...
	var e1 = effs.get(&"pilot_065_effect_01")
	if e1 == null:
	...
	for c in o0_conds:
		if String(c.get("op", &"")) == "CARD_COUNTER_IS":
			...
			if String(cp.get("key", &"")) == "pilot_065_intercept" and bool(cp.get("value", true)) == true and bool(cp.get("default_when_absent", false)) == true:
				o0_has_cbi = true
	if not o0_has_cbi:
		return "选项0 缺 CARD_COUNTER_IS(key=pilot_065_intercept,value=true,default_when_absent=true)"
	...
	if String(o0_p.get("key", &"")) != "pilot_065_intercept" or bool(o0_p.get("value", true)) != false:
	...
	if String(o1_p.get("key", &"")) != "pilot_065_intercept" or bool(o1_p.get("value", false)) != true:
		return "选项1 SET_CARD_COUNTER 应 value=true（启用）"

	var e2 = effs.get(&"pilot_065_effect_02")
	if e2 == null:
		return "缺 pilot_065_effect_02"
...
func test_p065_toggle_off_then_on() -> Variant:
	...
	var ef1 := await _fire_pilot_065_toggle(battle, pilot, s.mech, &"player")
	if ef1 == null:
	...
	var ef2 := await _fire_pilot_065_toggle(battle, pilot, s.mech, &"player")
	if ef2 == null:
...
func test_p065_toggle_off_no_trigger() -> Variant:
	...
	# 先禁用开关
	var ef := await _fire_pilot_065_toggle(battle, s.pilot_card, s.mech, &"player")
	if ef == null:
