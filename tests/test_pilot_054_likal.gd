## test_pilot_054_likal.gd - 莉卡尔（pilot_054，秩序 R）效果测试
##
...
func _setup_likal(battle, owner_id: StringName):
	...
	if mech == null:
		return {}
	var card = _make_instance(gs, cdb, "pilot_054_莉卡尔", owner_id)
	if card == null:
		return {}
...
func test_pilot_054_effect_definitions() -> Variant:
	...
	for c in e1.conditions:
		if String(c.get("op", &"")) == "EFFECT_ONCE_PER_TURN_AVAILABLE":
			...
			if String(c_p.get("once_per_turn_key", &"")) != "pilot_054_effect_01" or int(c_p.get("once_per_turn_max", 0)) != 2:
				return "effect_01 额度应为 pilot_054_effect_01 max=2（每回合2次）"
	...
	if String(mark_params.get("once_per_turn_key", &"")) != "pilot_054_effect_01":
		return "effect_01 MARK 额度 key 应 pilot_054_effect_01"
...
func test_pilot_054_buy_normal_confirm() -> Variant:
...
func test_pilot_054_buy_advanced_confirm() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_likal(battle, &"player")
...
func test_pilot_054_buy_cancel() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_likal(battle, &"player")
...
func test_pilot_054_owner_isolation_pvp3() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
...
func test_pilot_054_other_buy_no_trigger() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_likal(battle, &"player")
