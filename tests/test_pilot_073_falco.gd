## test_pilot_073_falco.gd - 法尔科（pilot_073，帝国 N）效果测试
##
...
func _setup_falco(battle, owner_id: StringName):
	...
	if mech == null:
		return {}
	var card = _make_instance(gs, cdb, "pilot_073_法尔科", owner_id)
	if card == null:
		return {}
...
func _fire_pilot_073(battle, pilot_card, mech, player_id: StringName) -> _Action:
	var src: Dictionary = {
...
func test_pilot_073_effect_definition() -> Variant:
	var effects: Dictionary = _ActionPilotEffects.build_pilot_effects()
	var e = effects.get(&"pilot_073_effect_01")
	if e == null:
		return "缺 pilot_073_effect_01"
	if e.mode != _TimingConst.MODE_DIRECT:
		return "mode 应 MODE_DIRECT 实=%s" % String(e.mode)
	if String(e.once_per_turn_key) != "pilot_073_effect_01":
		return "once_per_turn_key 应 pilot_073_effect_01"
	...
	if String(cm_p.get("store_result_key", &"")) != "pilot_073_effect_01_discard_ids":
		return "store_result_key 应 pilot_073_effect_01_discard_ids"
	...
	if String(sink_cfg.get("key", &"")) != "pilot_073_effect_01_drawn":
		return "_draw_result_sink.key 应 pilot_073_effect_01_drawn"
...
func test_pilot_073_full_flow_own_reserve() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_falco(battle, &"player")
	if s.is_empty():
		return "setup 失败（缺 pilot_073_法尔科）"
	...
	var ef = await _fire_pilot_073(battle, s.pilot_card, s.mech, &"player")
	if ef == null:
	...
	if not _in_action_discard(battle, sel1) or not _in_action_discard(battle, sel2):
		return "被弃行动牌应进行动弃牌堆"
	# 抽1高级装备 + 禁标签 + 牌堆-1
	var drawn: Array = ef.record.get(&"pilot_073_effect_01_drawn", [])
	if drawn.size() != 1:
...
func test_pilot_073_full_flow_enemy_reserve() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_falco(battle, &"player")
...
func test_pilot_073_cancel_no_cost() -> Variant:
	...
	if gs.deck_state.advanced_equipment_deck.size() != adv_before:
		return "取消不应抽牌"
	if not battle.context.timing_engine.is_once_per_turn_key_available(&"pilot_073_effect_01", s.pilot_card.instance_id, 1):
		return "取消不应消耗每回合1次"
	var ef2 = await _fire_pilot_073(battle, s.pilot_card, s.mech, &"player")
	if ef2 == null:
		return "取消中止后应可再触发"
	return true
...
func test_pilot_073_once_per_turn() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_falco(battle, &"player")
	...
	if hand.size() < 2:
		return "起手行动牌不足2张"
	var ef1 = await _fire_pilot_073(battle, s.pilot_card, s.mech, &"player")
	if ef1 == null:
		return "第1次未挂起"
	...
	if battle.context.timing_engine.is_once_per_turn_key_available(&"pilot_073_effect_01", s.pilot_card.instance_id, 1):
...
func test_pilot_073_insufficient_hand() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_falco(battle, &"player")
...
func test_pilot_073_tag_helpers() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var gs = battle.context.game_state
...
func test_pilot_073_set_sell_gates() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var gs = battle.context.game_state
...
func test_pilot_073_turn_start_clear() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var gs = battle.context.game_state
...
func test_pilot_073_round_start_clear_all() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var gs = battle.context.game_state
