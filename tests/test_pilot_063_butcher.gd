## test_pilot_063_butcher.gd - 布彻尔（pilot_063，联邦 N）效果测试
##
...
func _setup_butcher(battle, owner_id: StringName) -> Dictionary:
	...
	if mech == null:
		return {}
	var card = _make_instance(gs, cdb, "pilot_063_布彻尔", owner_id)
	if card == null:
		return {}
...
func test_pilot_063_effect_definition() -> Variant:
	var e1 = _ActionPilotEffects.build_pilot_effects().get(&"pilot_063_effect_01")
	if e1 == null:
		return "缺 pilot_063_effect_01"
	...
	var e2 = _ActionPilotEffects.build_pilot_effects().get(&"pilot_063_effect_02")
	if e2 == null:
		return "缺 pilot_063_effect_02"
...
func test_pilot_063_e1_gate_conditions() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_butcher(battle, &"player")
	if s.is_empty():
		return "setup 失败（缺 pilot_063_布彻尔）"
	var gs = s.gs
	var mech = s.mech
	var enemy_mech = gs.get_mech_for_player(&"enemy")
	...
	var eff1 = _ActionPilotEffects.build_pilot_effects().get(&"pilot_063_effect_01")
	...
	te.mark_once_per_turn_key_used(&"pilot_063_effect_01", s.pilot_card.instance_id)
	if te.can_trigger_active_effect(eff1, bind_ctx):
...
func test_pilot_063_e1_window_exempt() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_butcher(battle, &"player")
...
func test_pilot_063_e1_transform_attack() -> Variant:
	...
	if battle.context.timing_engine.is_once_per_turn_key_available(&"pilot_063_effect_01", cid, 1):
		return "发动后每回合额度应已消耗"
	# 燃料牌入弃牌堆（临时区链末）
...
func test_pilot_063_e1_cancel_no_cost() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_butcher(battle, &"player")
	...
	var te = battle.context.timing_engine
	if not te.is_once_per_turn_key_available(&"pilot_063_effect_01", cid, 1):
		return "前置：额度应可用"
...
func test_pilot_063_e2_responded_draw2() -> Variant:
	var setup := _setup_standard(_new_battle())
	if setup.is_empty():
		return "setup 失败"
	var battle = setup.battle
...
func test_pilot_063_e2_not_responded_discard_target() -> Variant:
	var setup := _setup_standard(_new_battle())
	if setup.is_empty():
		return "setup 失败"
	var battle = setup.battle
...
func test_pilot_063_e2_auto_discard_all() -> Variant:
	var setup := _setup_standard(_new_battle())
	if setup.is_empty():
		return "setup 失败"
	var battle = setup.battle
...
func test_pilot_063_e2_non_assault_no_trigger() -> Variant:
	var setup := _setup_standard(_new_battle())
	if setup.is_empty():
		return "setup 失败"
	var battle = setup.battle
...
func test_pilot_063_e2_transform_assault_triggers() -> Variant:
	var setup := _setup_standard(_new_battle())
	if setup.is_empty():
		return "setup 失败"
	var battle = setup.battle
