## test_pilot_063_lorne.gd - 洛尔恩（pilot_063，联邦 N）效果测试
##
...
func _setup(battle) -> Dictionary:
	...
	var pilot_card = _make_instance(gs, cdb, "pilot_063_洛尔恩", &"player")
	if pilot_card == null:
		return {}
...
func _opt_used(te, pilot_card) -> int:
	var key: String = "%s:%s" % [String(pilot_card.instance_id), "pilot_063_effect_01"]
...
func test_luoern_effect_definition() -> Variant:
	...
	if String(acts[0].get("params", {}).get("store_result_key", &"")) != &"pilot_063_fuel_ids":
		return "effect_01 应 store_result_key=pilot_063_fuel_ids"
...
func test_luoern_cover_convert_full_flow() -> Variant:
	...
	battle.context.timing_engine.resume_pending_effect(cover_action_id, {
		"selected_card_ids": [],
		"selected_extra_ids": ["pilot_063_effect_01"],
	})
	await _pump_frames(3)
