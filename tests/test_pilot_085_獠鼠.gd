## test_pilot_085_獠鼠.gd - 獠鼠（pilot_085，混乱 N，cost 5, attack_limit 2, action_card_limit 1）效果测试
##
...
func _setup_pilot_085(battle, owner_id: StringName):
	...
	var card = _make_instance(gs, cdb, "pilot_085_獠鼠", owner_id)
	if card == null:
		return null
...
## 取出獠鼠确认弹窗（choose_one_effect，effect_id=pilot_085_effect_01）；无则返回 {}。
func _pop_086_confirm(driver) -> Dictionary:
	for i in range(driver.popups.size()):
		var p: Dictionary = driver.popups[i]
		if String(p.input_params.get("effect_id", &"")) == "pilot_085_effect_01":
...
func test_pilot_085_effect_definitions() -> Variant:
	...
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var ids: Array = _ActionPilotEffects.get_effects_for_pilot(&"pilot_085_獠鼠", battle.context)
	var id_strs: Array = []
	for i in ids:
	...
	var gs = battle.context.game_state
	var cdb = battle.context.card_database
	var pdef = cdb.get_card(&"pilot_085_獠鼠")
	if pdef == null:
		return "缺 pilot_085_獠鼠 CardDef"
...
func test_pilot_085_confirm_lock_branch() -> Variant:
	var setup := _setup_standard(_new_battle())
	if setup.is_empty():
		return "setup 失败（缺 pilot_085_獠鼠）"
	var battle = setup.battle
	var driver = setup.driver
	var te = setup.te
	var gs = setup.gs
	...
	te._pending_effect[popup.action_id]["payload"]["pilot_085_forced_dice"] = 6
...
func test_pilot_085_cancel_no_branch() -> Variant:
	var setup := _setup_standard(_new_battle())
	if setup.is_empty():
		return "setup 失败"
	var battle = setup.battle
...
## 测试4：骰子1——我方机甲 +2 损伤。
func test_pilot_085_dice1_self_damage() -> Variant:
	var setup := _setup_standard(_new_battle())
	if setup.is_empty():
		return "setup 失败"
	var battle = setup.battle
	...
	if popup.is_empty():
		return "攻击应弹獠鼠确认窗"
	te._pending_effect[popup.action_id]["payload"]["pilot_085_forced_dice"] = 1
	te.resume_pending_effect(popup.action_id, {"chosen_option_index": 0})
	await _drain_all(battle, driver)
...
## 测试5：骰子2~3——我方抽2张行动牌。
func test_pilot_085_dice2_3_draw() -> Variant:
	var setup := _setup_standard(_new_battle())
	if setup.is_empty():
		return "setup 失败"
	var battle = setup.battle
	...
	if popup.is_empty():
		return "攻击应弹獠鼠确认窗"
	te._pending_effect[popup.action_id]["payload"]["pilot_085_forced_dice"] = 2
	te.resume_pending_effect(popup.action_id, {"chosen_option_index": 0})
	await _drain_all(battle, driver)
...
func test_pilot_085_dice4_5_auto_discard() -> Variant:
	var setup := _setup_standard(_new_battle())
	if setup.is_empty():
		return "setup 失败"
	var battle = setup.battle
	...
	if popup.is_empty():
		return "攻击应弹獠鼠确认窗"
	te._pending_effect[popup.action_id]["payload"]["pilot_085_forced_dice"] = 4
	te.resume_pending_effect(popup.action_id, {"chosen_option_index": 0})
	await _drain_all(battle, driver)
...
func test_pilot_085_dice4_5_choose_discard() -> Variant:
	var setup := _setup_standard(_new_battle())
	if setup.is_empty():
		return "setup 失败"
	var battle = setup.battle
	...
	if popup.is_empty():
		return "攻击应弹獠鼠确认窗"
	te._pending_effect[popup.action_id]["payload"]["pilot_085_forced_dice"] = 5
	te.resume_pending_effect(popup.action_id, {"chosen_option_index": 0})
...
func test_pilot_085_dual_strike_targets() -> Variant:
	var setup := _setup_standard(_new_battle())
	if setup.is_empty():
		return "setup 失败"
	var battle = setup.battle
	...
	if popup.is_empty():
		return "确认弹窗应 effect_id=pilot_085_effect_01"
