## test_pilot_062_kade.gd - 铠德（pilot_062，混乱 R）效果测试
##
## 铠德 1 个被动效果（LISTEN ATTACK_SETTLE priority30）：
##   「若发动的攻击被响应，则可以选择其一：抽2张行动牌/回复3动力/获得4金币。」
##
## 通用模块 pilot_062_*（不绑定机师）：
##   · GameState 存 pilot_062_queue（待处理触发）/ pilot_062_pending_choice（当前待选择）。
...
func _setup_kaide(battle, owner_id: StringName) -> Dictionary:
	...
	if mech == null:
		return {}
	var card = _make_instance(gs, cdb, "pilot_062_铠德", owner_id)
	if card == null:
		return {}
...
## 测试1：effect_01 定义正确
func test_pilot_062_effect_definition() -> Variant:
	var e = _ActionPilotEffects.build_pilot_effects().get(&"pilot_062_effect_01")
	if e == null:
...
## 返回 pending（pilot_062_pending_choice，非空）。失败返回错误串（非 Dictionary）。
func _attack_and_expect_prompt(setup) -> Variant:
	...
	var p = gs.pilot_062_pending_choice
	if p.is_empty():
...
func test_pilot_062_choice_draw2() -> Variant:
	...
	_ActionPilotEffects.pilot_062_choose(ctx, &"player", player_mech.mech_id, 0)
	await _pump_frames(6)
...
func test_pilot_062_choice_power() -> Variant:
	var setup := _setup_standard(_new_battle())
	if setup.is_empty():
		return "setup 失败"
	var battle = setup.battle
	...
	_ActionPilotEffects.pilot_062_choose(ctx, &"player", player_mech.mech_id, 1)
	await _pump_frames(6)
...
func test_pilot_062_choice_gold() -> Variant:
	var setup := _setup_standard(_new_battle())
	if setup.is_empty():
		return "setup 失败"
	var battle = setup.battle
	...
	_ActionPilotEffects.pilot_062_choose(ctx, &"player", player_mech.mech_id, 2)
	await _pump_frames(6)
...
func test_pilot_062_abandon_no_effect() -> Variant:
	var setup := _setup_standard(_new_battle())
	if setup.is_empty():
		return "setup 失败"
	var battle = setup.battle
...
func test_pilot_062_not_responded_no_trigger() -> Variant:
	var setup := _setup_standard(_new_battle())
	if setup.is_empty():
		return "setup 失败"
	var battle = setup.battle
	...
	if not gs.pilot_062_queue.is_empty():
		return "未响应攻击不应入队 实=%s" % str(gs.pilot_062_queue)
...
func test_pilot_062_dual_strike_queue_serial() -> Variant:
	var setup := _setup_standard(_new_battle())
	if setup.is_empty():
		return "setup 失败"
	var battle = setup.battle
	...
	if gs.pilot_062_queue.size() != 1:
		return "第2个 fork 触发应入队（queue=1） 实=%d" % gs.pilot_062_queue.size()
	...
	var p2 = gs.pilot_062_pending_choice
	if p2.is_empty():
		return "第1个选完应处理队列→弹第2个三选一"
	if gs.pilot_062_queue.size() != 0:
		return "第2个三选一弹出后队列应清空 实=%d" % gs.pilot_062_queue.size()
	# 放弃第2个（安全阀，只验证队列串行）
	_ActionPilotEffects.pilot_062_choose(ctx, &"player", player_mech.mech_id, -1)
	await _pump_frames(6)
	if not gs.pilot_062_pending_choice.is_empty():
