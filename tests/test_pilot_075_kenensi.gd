## test_pilot_075_kenensi.gd - 肯尼斯（pilot_075，帝国 N）效果测试
##
...
func _setup_kenensi(battle, owner_id: StringName):
	...
	if mech == null:
		return {}
	var card = _make_instance(gs, cdb, "pilot_075_肯尼斯", owner_id)
	if card == null:
		return {}
...
func _fire_pilot_075_effect1(battle, pilot_card, mech, player_id: StringName) -> _Action:
	var src: Dictionary = {
...
func test_pilot_075_effect_definitions() -> Variant:
	var effects: Dictionary = _ActionPilotEffects.build_pilot_effects()

	# ── effect_01（按钮1 DIRECT）──
	var e1 = effects.get(&"pilot_075_effect_01")
	if e1 == null:
		return "缺 pilot_075_effect_01"
	...
	for c in e1.conditions:
		...
		if c_op == "EFFECT_ONCE_PER_TURN_AVAILABLE":
			...
			if String(cp.get("once_per_turn_key", &"")) != "pilot_075_effect_01":
				return "e1 once_per_turn_key 应 pilot_075_effect_01"
	...
	if String(e1cm.get("store_result_key", &"")) != "pilot_075_discard_ids":
		return "e1 store_result_key 应 pilot_075_discard_ids"
	...
	if String(e1dis.get("card_ids", &"")) != "$runtime.pilot_075_discard_ids":
		return "e1 EXECUTE_DISCARD card_ids 应 $runtime.pilot_075_discard_ids"

	# ── effect_02（按钮2 LISTEN 被动）──
	var e2 = effects.get(&"pilot_075_effect_02")
	if e2 == null:
		return "缺 pilot_075_effect_02"
	...
	var e2a = effects.get(&"pilot_075_effect_02_auto")
	var e2b = effects.get(&"pilot_075_effect_02_apply")
	var e2c = effects.get(&"pilot_075_effect_02_consume")
	var e2d = effects.get(&"pilot_075_effect_02_turnend")
...
func test_pilot_075_discard_non_support_popup_draw() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_kenensi(battle, &"player")
	if s.is_empty():
		return "setup 失败（缺 pilot_075_肯尼斯）"
	...
	var ef = await _fire_pilot_075_effect1(battle, s.pilot_card, s.mech, &"player")
	if ef == null:
...
func test_pilot_075_popup_might_stack() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_kenensi(battle, &"player")
...
func test_pilot_075_effect1_once_limit() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_kenensi(battle, &"player")
	...
	var ef2 = await _fire_pilot_075_effect1(battle, s.pilot_card, s.mech, &"player")
	if ef2 != null:
...
func test_pilot_075_turn_end_discard_popup_draw() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_kenensi(battle, &"player")
...
## 测试4：弹窗取消 → 不抽不累加
func test_pilot_075_popup_cancel() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_kenensi(battle, &"player")
...
func test_pilot_075_support_auto_both() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_kenensi(battle, &"player")
...
func test_pilot_075_might_applies_any_weapon() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_kenensi(battle, &"player")
...
func test_pilot_075_settle_consumes() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_kenensi(battle, &"player")
...
func test_pilot_075_turn_after_end_clear() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_kenensi(battle, &"player")
...
func test_pilot_075_discard_condition_boundaries() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_kenensi(battle, &"player")
