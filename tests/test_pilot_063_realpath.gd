## test_pilot_063_realpath.gd - 洛尔恩（pilot_063）真实攻击路径复现
##
## 现有 test_pilot_063_luoern.gd 用假 _Action 直接 fire_timing + 手动注册 cover_effect1，
...
func test_no_cover_card_no_window() -> Variant:
	...
	for e in extras:
		if String(e.get("effect_id", "")) == "pilot_063_effect_01":
			found = true
	if not found:
		return "掩护窗口弹出但 extra_options 无 pilot_063_effect_01: %s" % str(extras)
	return true
...
func test_with_cover_card_window_has_extra() -> Variant:
	...
	for e in extras:
		if String(e.get("effect_id", "")) == "pilot_063_effect_01":
			found = true
	if not found:
		return "extra_options 无 pilot_063_effect_01: %s" % str(extras)
	return true
	...
	while steps < 400:
		...
		if not driver.popups.is_empty():
			...
			if ptype == "select_thrust_cards":
				...
				if eff_id != "pilot_063_effect_02":
					return "期望 pilot_063_effect_02 二选一弹窗，实际: %s" % eff_id
