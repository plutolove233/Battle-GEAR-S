## test_pilot_077_kaysh.gd - 卡修（pilot_077，帝国 N）效果测试
##
...
##   （pilot_077_attack_restore / counter / support_restore），每回合各1次互不影响。
...
func _setup_kaxiu(battle, owner_id: StringName):
	...
	var card = _make_instance(gs, cdb, "pilot_077_卡修", owner_id)
	if card == null:
		return null
...
func test_pilot_077_effect_definitions() -> Variant:
	var all_effects: Dictionary = _ActionPilotEffects.build_pilot_effects()
	var expect: Array = [
		{"eid": &"pilot_077_effect_01a", "type": "攻击", "amount": 5, "once": &"pilot_077_attack_restore", "hide": false},
		{"eid": &"pilot_077_effect_01b", "type": "迎击", "amount": 4, "once": &"pilot_077_counter_restore", "hide": true},
		{"eid": &"pilot_077_effect_01c", "type": "辅助", "amount": 3, "once": &"pilot_077_support_restore", "hide": true},
	]
	for exp in expect:
...
## 测试2：使用攻击牌 → 回复5动力
func test_pilot_077_attack_restores_5() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_kaxiu(battle, &"player")
	if s == null:
		return "setup 失败（缺 pilot_077_卡修）"
...
func test_pilot_077_counter_restores_4() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_kaxiu(battle, &"player")
...
## 测试4：使用辅助牌 → 回复3动力
func test_pilot_077_support_restores_3() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_kaxiu(battle, &"player")
...
func test_pilot_077_same_type_once_per_turn() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_kaxiu(battle, &"player")
...
func test_pilot_077_branches_independent() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_kaxiu(battle, &"player")
...
func test_pilot_077_other_player_use_no_trigger() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_kaxiu(battle, &"player")
...
func test_pilot_077_cross_turn_reset() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_kaxiu(battle, &"player")
