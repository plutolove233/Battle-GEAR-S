## test_pilot_067_hacke.gd - 骇客（pilot_067，联邦 N，cost 3, attack_limit 1, action_card_limit 4）
##
## 骇客效果（运行时机师效果走 ActionPilotEffects 新体系，通用模块 VIEW_RANDOM_OTHER_HAND_CARDS）：
##   effect_01（DIRECT 显示按钮，开关）：随时按弹"启用/禁用骇客技能"二选一（可取消）。
##     flag 存 card.counters["pilot_067_hack_switch"]（默认启用=true）。CHOOSE_ONE 两选项各带
...
func _setup_hacker(battle, owner_id: StringName):
	...
	if mech == null:
		return {}
	var card = _make_instance(gs, cdb, "pilot_067_骇客", owner_id)
	if card == null:
		return {}
...
func _fire_pilot_067_toggle(battle, pilot_card, mech, player_id: StringName) -> _Action:
	var src: Dictionary = {
		"card_instance_id": pilot_card.instance_id,
		"mech_id": mech.mech_id,
		"player_id": player_id,
		"effect_id": &"pilot_067_effect_01",
	}
...
## 读取机师牌 counters["pilot_067_hack_switch"]（不存在按默认 true）
func _hack_flag(pilot_card) -> bool:
	if pilot_card == null:
		return true
	if not "counters" in pilot_card:
		return true
	return bool(pilot_card.counters.get("pilot_067_hack_switch", true))
...
func test_p066_definitions() -> Variant:
	...
	var e1 = effs.get(&"pilot_067_effect_01")
	if e1 == null:
		return "缺 pilot_067_effect_01"
	...
	if String(o0_p.get("key", &"")) != "pilot_067_hack_switch" or bool(o0_p.get("value", true)) != false:
	...
	if String(o1_p.get("key", &"")) != "pilot_067_hack_switch" or bool(o1_p.get("value", false)) != true:
		return "选项1 SET_CARD_COUNTER 应 value=true（启用）"

	var e2 = effs.get(&"pilot_067_effect_02")
	if e2 == null:
		return "缺 pilot_067_effect_02"
	...
	for c in e2.conditions:
		...
		if cop == "EFFECT_ONCE_PER_TURN_AVAILABLE":
			...
			if String(cp.get("once_per_turn_key", &"")) != "pilot_067_effect_02":
				return "EFFECT_ONCE_PER_TURN_AVAILABLE once_per_turn_key 应 pilot_067_effect_02"
	...
	if String(p2.get("once_per_turn_key", &"")) != "pilot_067_effect_02":
		return "effect_02 params once_per_turn_key 应 pilot_067_effect_02"
	if String(p2.get("store_target_key", &"")) != "pilot_067_target_id":
		return "effect_02 params store_target_key 应 pilot_067_target_id"

	# 旧 CardEffect 定义已删（迁移至 ActionPilotEffects）
	if _GeneratedPilotEffects.build_pilot_effects().has(&"pilot_067_effect_01"):
		return "旧 GeneratedPilotEffects 不应再有 pilot_067_effect_01"
	...
	if s.is_empty():
		return "setup 失败（缺 pilot_067_骇客 或 effect_ids 未更新）"
	var te = s.te
	var found_direct := false
	var found_listen := false
	for entry in te.permanent_listeners.get(&"pilot_067_effect_01", []):
		var eff = entry.get("effect", null)
		if eff != null and String(eff.effect_id) == "pilot_067_effect_01":
			found_direct = true
	for entry in te.permanent_listeners.get(_TimingConst.BASIC_MOVE_AFTER, []):
		var eff = entry.get("effect", null)
		if eff != null and String(eff.effect_id) == "pilot_067_effect_02":
			found_listen = true
...
func test_p066_toggle_off_then_on() -> Variant:
	...
	var ef1 := await _fire_pilot_067_toggle(battle, pilot, s.mech, &"player")
	if ef1 == null:
	...
	var ef2 := await _fire_pilot_067_toggle(battle, pilot, s.mech, &"player")
	if ef2 == null:
...
func test_p066_peek_attack_support() -> Variant:
	...
	# 标记已消耗1次（本回合剩余1次）
	if not te.is_once_per_turn_key_available(&"pilot_067_effect_02", s.pilot_card.instance_id, 2):
...
func test_p066_switch_off_no_trigger() -> Variant:
	...
	battle.context.action_ui_bridge.context = battle.context
	# 先禁用开关
	var ef := await _fire_pilot_067_toggle(battle, s.pilot_card, s.mech, &"player")
	if ef == null:
