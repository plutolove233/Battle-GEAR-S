## test_pilot_088_conquer.gd - 征服（pilot_088，混乱 N，cost 3, attack_limit 1, action_card_limit 3）效果测试
##
...
func _setup_pilot_088(battle, owner_id: StringName):
	var gs = battle.context.game_state
	var cdb = battle.context.card_database
	var mech = gs.get_mech_for_player(owner_id)
	var card = _make_instance(gs, cdb, "pilot_088_征服", owner_id)
	if card == null:
		return null
...
func _fire_pilot_088(battle, pilot_card, mech, player_id: StringName, effect_id: StringName) -> _Action:
	var src: Dictionary = {
...
func _setup_standard(battle) -> Dictionary:
	var s = _setup_pilot_088(battle, &"player")
	if s == null:
		return {}
	var gs = s.gs
...
func test_pilot_088_effect_definition() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var ids: Array = _ActionPilotEffects.get_effects_for_pilot(&"pilot_088_征服", battle.context)
	var id_strs: Array = []
	for i in ids:
		id_strs.append(String(i))
	if not id_strs.has("pilot_088_effect_01"):
		return "effect_ids 应含 pilot_088_effect_01 实=%s" % str(id_strs)
	var e1 = _ActionPilotEffects.build_pilot_effects().get(&"pilot_088_effect_01")
	if e1 == null:
		return "缺 pilot_088_effect_01"
	if e1.mode != _TimingConst.MODE_DIRECT:
		return "e1 mode 应 DIRECT 实=%s" % String(e1.mode)
	if e1.once_per_turn_key != &"pilot_088_effect_01" or int(e1.once_per_turn_max) != 1:
...
## 次数消耗
func test_pilot_088_match_discard_rest() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var setup = _setup_standard(battle)
	if setup.is_empty():
		return "setup 失败（缺 pilot_088_征服）"
	var gs = setup["gs"]
	...
	if hand.size() != 3:
		return "补3张攻击牌失败"
	var ef = await _fire_pilot_088(battle, pilot_card, player_mech, &"player", &"pilot_088_effect_01")
	if ef == null:
	...
	# 宣言"攻击"（三选一确定，不可取消）
	te.resume_pending_effect(ef.action_id, {"pilot_088_declared_type": "攻击"})
	await _wait_effect_fire_done(battle, &"pilot_088_effect_01", 100)
	...
	var ef2 = await _fire_pilot_088(battle, pilot_card, player_mech, &"player", &"pilot_088_effect_01")
	if ef2 != null:
...
func test_pilot_088_mismatch_discard_shown() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var setup = _setup_standard(battle)
...
func test_pilot_088_target_cancel_no_cost() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var setup = _setup_standard(battle)
	...
	await _pump_frames(5)
	te.resume_pending_effect(ef2.action_id, {"pilot_088_declared_type": "攻击"})
	await _wait_effect_fire_done(battle, &"pilot_088_effect_01", 100)
	return true


## 测试6：范围/目标过滤——3格外有牌 or 3格内有牌机甲为空（无候选）→ 条件不满足，effect_fire 不挂起
func test_pilot_088_no_candidate() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot_088(battle, &"player")
	...
	_clear_action_hand(battle, &"enemy")
	_give_action_cards_of_type(battle, &"enemy", "action_001_进攻", 3)
	var ef = await _fire_pilot_088(battle, s.card, s.mech, &"player", &"pilot_088_effect_01")
	if ef != null:
	...
	_place_mech(battle, enemy_mech.mech_id, 4, 2)
	_clear_action_hand(battle, &"enemy")
	var ef2 = await _fire_pilot_088(battle, s.card, s.mech, &"player", &"pilot_088_effect_01")
	if ef2 != null:
