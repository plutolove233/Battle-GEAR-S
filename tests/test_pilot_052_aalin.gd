## test_pilot_052_aalin.gd - 亚林（pilot_052，秩序 R）效果测试
##
...
func _setup_yalin(battle, owner_id: StringName):
	...
	if mech == null:
		return {}
	var card = _make_instance(gs, cdb, "pilot_052_亚林", owner_id)
	if card == null:
		return {}
...
func test_pilot_052_effect_definitions() -> Variant:
	...
	for c in e1.conditions:
		if String(c.get("op", &"")) == "EFFECT_ONCE_PER_TURN_AVAILABLE":
			...
			if String(c_p.get("once_per_turn_key", &"")) != "pilot_052_effect_01" or int(c_p.get("once_per_turn_max", 0)) != 2:
				return "effect_01 额度应为 pilot_052_effect_01 max=2（每回合2次）"
	...
	if String(mark_params.get("once_per_turn_key", &"")) != "pilot_052_effect_01":
		return "effect_01 MARK 额度 key 应 pilot_052_effect_01"
	...
	var e1b = effects.get(&"pilot_052_effect_01b")
	if e1b == null:
		return "缺 pilot_052_effect_01b"
	...
	for c in e1b.conditions:
		if String(c.get("op", &"")) == "EFFECT_ONCE_PER_TURN_AVAILABLE":
			...
			if String(cb_p.get("once_per_turn_key", &"")) != "pilot_052_effect_01" or int(cb_p.get("once_per_turn_max", 0)) != 2:
...
func test_pilot_052_set_equip_confirm() -> Variant:
...
func test_pilot_052_set_equip_cancel() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_yalin(battle, &"player")
...
func test_pilot_052_discard_equip_confirm() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_yalin(battle, &"player")
...
func test_pilot_052_hand_bonus_clears_next_turn() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_yalin(battle, &"player")
...
func test_pilot_052_owner_isolation_pvp3() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
