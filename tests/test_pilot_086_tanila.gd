## test_pilot_086_tanila.gd - 塔妮拉（pilot_086，混乱 N，cost 3, attack_limit 1, action_card_limit 4）效果测试
##
...
func _setup_pilot_086(battle, owner_id: StringName):
	...
	var card = _make_instance(gs, cdb, "pilot_086_塔妮拉", owner_id)
	if card == null:
		return null
...
func _fire_pilot_086(battle, pilot_card, mech, player_id: StringName, effect_id: StringName) -> _Action:
	var src: Dictionary = {
...
func test_pilot_086_effect_definitions() -> Variant:
	...
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	# JSON effect_ids 全注册
	var ids: Array = _ActionPilotEffects.get_effects_for_pilot(&"pilot_086_塔妮拉", battle.context)
	var id_strs: Array = []
	for i in ids:
		id_strs.append(String(i))
	for need in ["pilot_086_effect_01", "pilot_086_effect_02"]:
	...
	if e1.once_per_turn_key != &"pilot_086_effect_01" or int(e1.once_per_turn_max) != 2:
	...
	var e2 = _ActionPilotEffects.build_pilot_effects().get(&"pilot_086_effect_02")
	if e2 == null:
		return "缺 pilot_086_effect_02"
...
func test_pilot_086_effect01_full_flow() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var setup = _setup_standard_e1(battle)
	if setup.is_empty():
		return "setup 失败（缺 pilot_086_塔妮拉）"
	var gs = setup["gs"]
	...
	var ef = await _fire_pilot_086(battle, pilot_card, player_mech, &"player", &"pilot_086_effect_01")
	if ef == null:
	...
	await _wait_effect_fire_done(battle, &"pilot_086_effect_01", 80)
	# 交出的牌进敌方手牌
	...
	var ef2 = await _fire_pilot_086(battle, pilot_card, player_mech, &"player", &"pilot_086_effect_01")
	if ef2 == null:
	...
	# 第3次应不能挂起（次数耗尽）
	_give_action_cards(battle, &"player", 1)
	var ef3 = await _fire_pilot_086(battle, pilot_card, player_mech, &"player", &"pilot_086_effect_01")
	if ef3 != null:
...
func test_pilot_086_effect01_mech_cancel() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var setup = _setup_standard_e1(battle)
	...
	# 第四次应被拒（累计2次 == max=2）
	_give_action_cards(battle, &"player", 1)
	var ef4 = await _fire_pilot_086(battle, pilot_card, player_mech, &"player", &"pilot_086_effect_01")
	if ef4 != null:
...
func test_pilot_086_transfer_steal_apply_jiao() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot_086(battle, &"player")
	...
	if not gs.players.get(&"enemy").action_hand.has(c1_inst_id):
		return "转移后敌方手牌应含 c1"
	if not _ActionPilotEffects.pilot_086_card_has_any_jiao(gs.get_card(c1_inst_id)):
		return "转移交牌应打交标签"
	...
	if not gs.players.get(&"enemy").action_hand.has(c2_inst_id):
		return "偷牌后敌方手牌应含 c2"
	if not _ActionPilotEffects.pilot_086_card_has_any_jiao(gs.get_card(c2_inst_id)):
		return "偷牌也应打交标签"
	return true


## 测试5："交"标签使用判定——使用（temp_zone->弃牌堆）双方各抽1（使用方先，塔妮拉后）；直接弃置不抽
func test_pilot_086_jiao_use_triggers_draw() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot_086(battle, &"player")
	...
	_ActionPilotEffects.pilot_086_tag_jiao(c1_card, &"player")
	if not _ActionPilotEffects.pilot_086_card_has_any_jiao(c1_card):
		return "c1 应带交标签"
	...
	if _ActionPilotEffects.pilot_086_card_has_any_jiao(c1_card):
	...
	gs.cards[c2_inst_id] = c2_card
	_ActionPilotEffects.pilot_086_tag_jiao(c2_card, &"player")
	...
	if _ActionPilotEffects.pilot_086_card_has_any_jiao(c2_card):
		return "使用入弃牌堆后交标签应消失"
	return true


## 测试6：unset_pilot 离场清塔妮拉名下全部"交"标签
func test_pilot_086_unset_clears_jiao() -> Variant:
	var battle := _new_battle()
	if battle == null or battle.context == null:
		return "battle 初始化失败"
	var s = _setup_pilot_086(battle, &"player")
	...
	_ActionPilotEffects.pilot_086_clear_all_jiao_for_player(gs, &"player")
