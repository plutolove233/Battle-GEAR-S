...
## 检查所有费用是否可支付
static func can_pay_all(binding, payload: Dictionary, costs: Array, ctx) -> bool:
	if costs.is_empty():
		return true
	for cost in costs:
...
static func pay_all(binding, payload: Dictionary, costs: Array, ctx) -> void:
	for cost in costs:
...
static func needs_discard_select(binding, payload: Dictionary, costs: Array, ctx) -> Dictionary:
	for cost in costs:
